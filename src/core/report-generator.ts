...
export function generateBugFeatureMd(
  session: Session,
  bugs: Bug[],
  features: Feature[]
): string {
  ...
  lines.push(`# Bugs & Features — ${session.name}`);
  lines.push('');
  lines.push(`> Session ID: \`${session.id}\`  `);
  lines.push(`> Generated: ${new Date().toISOString()}${session.project ? `  \n> Project: ${session.project}` : ''}`);
  lines.push('');

  if (sortedBugs.length > 0) {
    lines.push(`## Bugs (${sortedBugs.length})`);
    lines.push('');
    lines.push('| Priority | Status | Title | URL | Selector |');
    lines.push('|---|---|---|---|---|');
    for (const bug of sortedBugs) {
      const sel = bug.elementSelector ? `\`${bug.elementSelector}\`` : '—';
      lines.push(`| ${bug.priority} | ${bug.status} | ${bug.title} | ${bug.url} | ${sel} |`);
    }
    lines.push('');
    ...
    lines.push('## Bugs');
    lines.push('');
    lines.push('_No bugs logged in this session._');
    lines.push('');
  }
  ...
  if (features.length > 0) {
    ...
    lines.push('');
    lines.push('| Type | Status | Sprint | Title |');
    lines.push('|---|---|---|---|');
    for (const feat of features) {
      lines.push(`| ${feat.featureType} | ${feat.status} | ${feat.sprintRef ?? '—'} | ${feat.title} |`);
    }
    lines.push('');
    ...
    for (const feat of features) {
      ...
      if (feat.sprintRef) {
        lines.push(`- **Sprint:** ${feat.sprintRef}`);
      }
      lines.push(`- **URL:** ${feat.url}`);
      if (feat.description) {
        lines.push('');
    ...
    lines.push('## Features');
    lines.push('');
    lines.push('_No features logged in this session._');
    lines.push('');
  }
